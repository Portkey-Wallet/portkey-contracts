...
public partial class CAContract : CAContractImplContainer.CAContractImplBase
{
    ...
    public override KidPublicKeyOutput GetFacebookPublicKeyByKid(StringValue input)
    {
        ...
        return new KidPublicKeyOutput
        {
            ...
            PublicKeyChunks = { State.PublicKeysChunksByKid[GuardianType.OfFacebook][input.Value].PublicKeyChunks }
        };
    }
    ...
    public override Empty HandleOracleFulfillment(HandleOracleFulfillmentInput input)
    {
        ...
        else if (input.TraceId.Equals(HashHelper.ComputeFrom(State.OidcProviderData[GuardianType.OfFacebook].Issuer)))
        {
            foreach (var keyDto in response.Keys)
            {
                SetPublicKeyAndChunks(GuardianType.OfFacebook, keyDto.Kid, keyDto.N);
            }
        }
        else
        {
